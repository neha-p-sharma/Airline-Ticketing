...
-- Q2. FIND THE TTOAL NUMBER OF TICKETS SOLD PER AIRLINE
...
ON F.Destination=A2.AirportID
WHERE AL.Name= 'Indigo'

--Q4. FOR EACH AIRPORT, SHOW THE TOP AIRLINE BY NUMBER OF FLIGHTS DEPARTED FROM THERE
WITH FlightsCount as
(
	SELECT 
		A.Name AS Airport_name,
		AL.Name AS Airline_Name,
		COUNT(*) as No_of_Departures,
		ROW_NUMBER() OVER(PARTITION BY A.AirportID ORDER BY COUNT(*) DESC)AS rn
	FROM FLIGHTS F
	INNER JOIN Airlines AL
	ON F.AirlineID=AL.AirlineID
	INNER JOIN Airports A
	ON F.Origin=A.AirportID
	GROUP BY  A.AirportID, A.Name,AL.Name
)
...
SELECT FlightID,
	DepartureTime,
	ArrivalTime,
	CAST((DATEDIFF(MINUTE, DepartureTime,ArrivalTime) / 60.0)AS DECIMAL(10,2)) AS Duration_Hours
FROM Flights
)
SELECT *,
	CASE WHEN Duration_Hours <= 2 THEN 'SHORT'
...
-- Q7. FIND FLIGHTS WITH THE HIGHEST PRICE TICKET SOLD FOR EACH ROUTE (ORIGIN + DESCTINATION)
...
ON RT.Origin= A1.AirportID
INNER JOIN Airports A2
ON RT.Destination=A2.AirportID
WHERE rn = 1

-- Q8. FIND THE HIGHEST- SPENDING PASSENGER IN EACH FREQUENT FLYER STATUS GROUP

WITH SpendingByPassengers AS
(
SELECT 
	P.PassengerID,
	P.Name,
...
(
	SELECT *,
	RANK() OVER (PARTITION BY FrequentFlyerStatus ORDER BY Total_Spend DESC)AS rn
	FROM SpendingByPassengers
)
SELECT 
	Name,
	FrequentFlyerStatus,
